...
}//end of pokedesk object


var poke_param=pokemon_details(window.prompt("Enter the name of a pokemon to find its details")) //function call
function pokemon_details(poke_param) {
 if (poke_param==null || poke_param=='' || poke_param==undefined) {
  alert("Please enter a valid name")
 }else{
   for(x in pokedesk.pokemon) {
    if(poke_param==pokedesk.pokemon[x].name) {
      var obj=JSON.stringify(pokedesk.pokemon[x])
      alert(obj)
    }//inner if loop end
  }//for loop ends
}//else loop ends
}//pokemon_details funtion ends
   


var poke_param=evolution_of_whome(window.prompt("Enter the name of pokemon to find its prev_evolution pokemon"))//function call
function evolution_of_whome(poke_param) {
 if (poke_param==null || poke_param=='' || poke_param==undefined) {
   alert("Please enter a valid name")
 }else{
    for(x in pokedesk.pokemon) {
      for(y in pokedesk.pokemon[x].next_evolution) {
        if(poke_param==pokedesk.pokemon[x].next_evolution[y].name) {
          alert(pokedesk.pokemon[x].next_evolution[y].name +" is the evolution of "+ pokedesk.pokemon[x].name)
        }//inner if loop ends
      }//inner for loop ends
    }//outer for loop ends
  }//else ends
}//evolution_of_whome function ends




var poke_param=weakness_of_whome(window.prompt("Enter any weakness and see which pokemon has the same "))//function call
function weakness_of_whome(poke_param) {
 if (poke_param==null || poke_param=='' || poke_param==undefined) {
   alert("Please enter a valid name")
 }else{
    for(x in pokedesk.pokemon) {
      for(y in pokedesk.pokemon[x].weaknesses) {
        if(poke_param==pokedesk.pokemon[x].weaknesses[y]) {
          for(z in pokedesk.pokemon[x].weaknesses[y]) {
            alert(pokedesk.pokemon[x].name +" has "+ pokedesk.pokemon[x].weaknesses[y] +" weakness")
            break;
          }//innermost for loop ends
        }//inner if loop ends
      }//inner for loop ends
    }//outer for loop ends
  }//else ends
}//weakness_of_whome function ends
